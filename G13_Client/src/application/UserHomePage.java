...
public class UserHomePage implements Initializable {
	...
	@FXML // fx:id="user"
	private Text user; // Value injected by FXMLLoader
	...
	void Search(ActionEvent event) throws IOException {
		String msg = "showsearch#" + WhatToSearch.getText();
		Connect.client.handleMessageFromClientUI(msg);
		String[] m = Connect.client.servermsg.split("@");
		if ("ClientSearch".equals(m[0])) {
			String[] result = m[1].split("#");
			FXMLLoader loader = new FXMLLoader(getClass().getResource("ClientSearch.fxml"));
			AnchorPane root = (AnchorPane) loader.load();
			ClientSearch search = loader.getController();
			Image im = new Image("images/background.jpg");
			search.setimage(im);
			search.SeTEmail(TheEmail);
	...
	void LogOut(ActionEvent event) throws IOException {
		String message = "Userlogout#" + TheEmail;
		if (TheEmail.equals("")) {
			JOptionPane.showMessageDialog(null, "One or more files are empty!! ");
		} else {
			Connect.client.handleMessageFromClientUI(message);
			if (Connect.client.servermsg != null && "Userlogout".equals(Connect.client.servermsg)) {
